...
export function renderPage(cards) {
	renderContainer();  // Отображаем контейнер для карточек
	cards.forEach((card) => {
		renderCard(card);  // Отображаем каждую карточку
	});
}

export function toggleFavouriteIcon(elementIcon, isFaved) {
	if (isFaved) {
		elementIcon.classList.add('card__like--active');  // Добавляем активный класс, если элемент в избранном
	} else {
		elementIcon.classList.remove('card__like--active');  // Удаляем активный класс, если элемент не в избранном
	}
}
